...
```{r}
 KNN.decision <- function(Lag.new, Lag.data, K = 5) {
   
   n <- nrow(Lag.data)
   
   stopifnot(ncol(Lag.data) == 5, length(Lag.new) == 5, K <= n)
   
   dists       <- sqrt((Lag.new[1]-Lag.data[,1])^2+(Lag.new[2]-Lag.data[,2])^2
                       +(Lag.new[3]-Lag.data[,3])^2+(Lag.new[4]-Lag.data[,4])^2
                       +(Lag.new[5]-Lag.data[,5])^2)
...
lag.data <- cbind(train$Lag1,train$Lag2,train$Lag3,train$Lag4,train$Lag5)
prediction <- rep(NA,nrow(test))
for (i in 1:nrow(test)){
   lag.new <- cbind(test$Lag1,test$Lag2,test$Lag3,test$Lag4,test$Lag5)[i,]
   prediction[i] <- KNN.decision(Lag.new=lag.new,Lag.data=lag.data,K=5)
   
}

test.error <- mean(prediction!=test$Direction); test.error
...
for (i in 1:nrow(test)){
   lag.new <- cbind(test$Lag1,test$Lag2,test$Lag3,test$Lag4,test$Lag5)[i,]
   prediction[i] <- KNN.decision(Lag.new=lag.new,Lag.data=lag.data,K=3)
}

test.error2 <- mean(prediction!=test$Direction); test.error2
```
K=3 has a slightly lower error rate
...
ERR <- rep(NA,9)
for (j in 1:9){
   test <- Weekly[which(fold==j),]
   train <- Weekly[which(fold!=j),]
   lag.data <- cbind(train$Lag1,train$Lag2,train$Lag3,train$Lag4,train$Lag5)
   ...
   ERR[j] <- mean(prediction!=test$Direction)
}
CV.err5 <- mean(ERR); CV.err5
```
...
for (j in 1:9){
   ...
   for (i in 1:nrow(test)){
   lag.new <- cbind(test$Lag1,test$Lag2,test$Lag3,test$Lag4,test$Lag5)[i,]
   prediction[i] <- KNN.decision(Lag.new=lag.new,Lag.data=lag.data,K=1)
   }
   
   ERR[j] <- mean(prediction!=test$Direction)
}
CV.err1 <- mean(ERR); CV.err1
```
...
for (j in 1:9){
   ...
   for (i in 1:nrow(test)){
   lag.new <- cbind(test$Lag1,test$Lag2,test$Lag3,test$Lag4,test$Lag5)[i,]
   prediction[i] <- KNN.decision(Lag.new=lag.new,Lag.data=lag.data,K=7)
   }
   
   ERR[j] <- mean(prediction!=test$Direction)
}
CV.err7 <- mean(ERR); CV.err7 
```

K=7 has the lowest test error rate.