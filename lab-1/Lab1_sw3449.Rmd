...
sd(normal100)
```
The sample mean and sample standard deviation are not exactly equal to 0 and 1 because there exists randomness when drawing the random variables, and when sample size is not large enough there is a larger variance.
...
length(sample_means)
```
Pattern: The sample means are getting closer to 0 as the number of random variables increases.
...
```{r}
meanOfCol <- colSums(normal1mil_mat,dims=1)/100
```
...
This histogram approximates normal distribution.
